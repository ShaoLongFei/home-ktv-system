...
- [x] **PROD-03**: Mobile 控制台的搜索、点歌、队列、当前播放、在线补歌和操作按钮在手机宽度下布局稳定，文字居中且不溢出。
- [x] **PROD-04**: Admin 后台的导入、歌曲、房间和在线任务视图在信息密度、按钮状态、刷新反馈和异常提示上保持一致。
- [x] **PROD-05**: UI 回归测试覆盖关键中文文案、关键按钮可点击性、状态变化和空/错误状态展示。

### Code Structure & Logic

- [x] **QUAL-01**: Mobile、Admin、TV 的 API client、状态 hook、i18n 和展示组件边界清晰，避免页面组件承载过多业务逻辑。
