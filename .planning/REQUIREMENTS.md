...
- [ ] **TVUX-01**: TV 端在空闲、播放中、加载中、播放失败、重连恢复和播放器冲突状态下都有清楚且一致的视觉状态。
- [ ] **TVUX-02**: TV 播放中界面可以稳定展示当前歌曲、下一首、原唱/伴唱模式、播放状态和 `mm:ss / mm:ss` 时间信息。
- [ ] **TVUX-03**: 当浏览器首次播放需要用户手势时，TV 端提供明确的可操作提示，并在用户完成操作后恢复正常播放流程。
- [ ] **TVUX-04**: 原唱/伴唱切换、切歌、播放失败回退和重连恢复都能在 TV 端给出短暂但不干扰唱歌的反馈。
- [ ] **TVUX-05**: TV 端布局在客厅电视距离下可读，二维码、歌曲信息、状态文案和时间信息不会互相遮挡。

### Productized UI

- [ ] **PROD-01**: Admin、Mobile、TV 三端默认中文文案完整、一致，并保留可切换到英文的能力。
- [ ] **PROD-02**: Admin、Mobile、TV 三端的空状态、错误状态、加载状态和成功反馈使用统一语气和一致的信息层级。
- [ ] **PROD-03**: Mobile 控制台的搜索、点歌、队列、当前播放、在线补歌和操作按钮在手机宽度下布局稳定，文字居中且不溢出。
- [ ] **PROD-04**: Admin 后台的导入、歌曲、房间和在线任务视图在信息密度、按钮状态、刷新反馈和异常提示上保持一致。
...
- [ ] **QUAL-02**: 播放控制命令、房间快照、在线任务更新和错误处理的前后端状态流有统一的命名、结果处理和测试覆盖。
- [ ] **QUAL-03**: TV 播放运行时的首播、切换、失败、重连和冲突逻辑有可读的模块边界和针对性回归测试。
- [ ] **QUAL-04**: 项目移除明显重复、过时或调试遗留代码，并保留必要的开发脚本和文档入口。
...
|-------------|-------|--------|
| TVUX-01 | Phase 9 | Pending |
| TVUX-02 | Phase 9 | Pending |
| TVUX-03 | Phase 9 | Pending |
| TVUX-04 | Phase 9 | Pending |
| TVUX-05 | Phase 9 | Pending |
...
*Last updated: 2026-05-09 after v1.1 audit gap planning*
