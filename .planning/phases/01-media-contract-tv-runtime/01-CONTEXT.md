...
本阶段当前边界是：定义单房间 `living-room` 的可播放媒体契约、TV Player 运行时契约、播放器上报/恢复/冲突处理语义，以及服务端向 TV 下发播放目标状态的主链路。

基于本次讨论，还需特别注意一件事：用户已明确把“**播放中原唱 / 伴唱切换能力**”视为“资源是否算正式可唱”的核心标准，而这与当前 roadmap / requirements 中把“播放中切换”后置到后续阶段的设定冲突。进入正式 planning 前，必须先把这条冲突当成硬约束显式处理，而不是默认沿用现有 roadmap。
...
### Roadmap Alignment
- **D-09:** 当前 roadmap / requirements 把“播放中原唱 / 伴唱切换”放在后续阶段，但用户已明确这是正式可唱体验的前置条件；**planning 前必须先修正这条冲突**，不能继续把该能力当成 v2 需求忽略。
