# Requirements Archive: v1.2 真实 MV 歌库

**Archived:** 2026-05-14
**Status:** SHIPPED

This file is archived. Future active requirements will be created by `$gsd-new-milestone`.
...
- [x] **MEDIA-01**: User can store one MKV/MPG/MPEG MV file as one song candidate, without creating duplicate song records for the same physical file.
- [x] **MEDIA-02**: User can see whether a real MV file is ingestable, playable, review-required, or unsupported, with explicit reasons when it is not queueable.
- [x] **MEDIA-03**: User can preserve source media facts including container, duration, video codec, audio tracks, file size, and metadata provenance.
- [x] **MEDIA-04**: Future Android TV playback can reuse platform-neutral catalog/player fields without v1.2 implementing an Android TV app.
